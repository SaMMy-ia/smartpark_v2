...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Validações
    if (empty($marcaVeiculo)) $errors[] = 'Marca do veículo é obrigatória.';
    if (empty($matricula)) {
        $errors[] = 'Matrícula é obrigatória.';
    } elseif (!validarMatricula($matricula)) {
        $errors[] = 'Formato de matrícula inválido. Use formato: ABC-1234';
    }
    if (empty($proprietario)) $errors[] = 'Nome do proprietário é obrigatório.';
    if (empty($contacto)) {
    ...
    if (empty($errors)) {
        ...
        ");
        $stmt->execute([
            $_SESSION['user_id'], $vagaId, $veiculoId, $dataInicio, $dataFim, $valorTotal,
            $marcaVeiculo, $matricula, $proprietario, $contacto
        ]);
    ...
    <form method="POST" id="reservaForm" class="space-y-6">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
            <h2 class="text-xl font-semibold text-gray-900 dark:text-white mb-4">
                <span class="inline-flex items-center justify-center w-8 h-8 rounded-full bg-primary dark:bg-secondary text-white mr-2">1</span>
                Selecione o Horário
            </h2>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
            <h2 class="text-xl font-semibold text-gray-900 dark:text-white mb-4">
                <span class="inline-flex items-center justify-center w-8 h-8 rounded-full bg-primary dark:bg-secondary text-white mr-2">2</span>
                Escolha a Vaga
            </h2>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
            <h2 class="text-xl font-semibold text-gray-900 dark:text-white mb-4">
                <span class="inline-flex items-center justify-center w-8 h-8 rounded-full bg-primary dark:bg-secondary text-white mr-2">3</span>
            ...
            ?>
                ...
                <div class="flex items-center justify-between mb-2">
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">
                        <i class="fas fa-list mr-1"></i> Selecione um dos seus veículos
                    </label>
                    <a href="veiculos.php" class="text-sm font-bold text-primary dark:text-secondary hover:underline">
                        <i class="fas fa-plus-circle mr-1"></i> Cadastrar novo veículo
                    </a>
                </div>
                <select id="vehicleSelect" name="veiculo_id_sel" class="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary">
                    <option value="">-- Cadastrar novo veículo / Outra viatura --</option>
                    <?php foreach ($userVehicles as $veh): ?>
                        <option value="<?php echo $veh['id']; ?>" 
                                data-marca="<?php echo e($veh['marca']); ?>" 
                                data-matricula="<?php echo e($veh['matricula']); ?>"
                                data-proprietario="<?php echo e($veh['proprietario_nome']); ?>"
                                data-contacto="<?php echo e($veh['contacto']); ?>">
                            <?php echo e($veh['marca']); ?> - <?php echo e($veh['matricula']); ?>
                        </option>
                    <?php endforeach; ?>
                </select>
                <p class="text-xs text-gray-500 mt-1">Ao selecionar um veículo, os campos abaixo serão preenchidos automaticamente.</p>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" 
                           ...
                           value="<?php echo e($_POST['marca_veiculo'] ?? ''); ?>"
                           class="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary">
                </div>
                ...
                <div>
                    ...
                    <input type="text" 
                           ...
                           id="matricula" 
                           required
                           placeholder="ABC-123-DE (Formato Moçambicano)"
                           maxlength="11"
                           pattern="[A-Z]{3}-[0-9]{3}-[A-Z]{2}"
                           title="Formato: ABC-123-DE (3 letras, 3 números, 2 letras)"
...
document.getElementById('vehicleSelect').addEventListener('change', function() {
    ...
    if (this.value) {
        ...
        const prop = selectedOption.getAttribute('data-proprietario');
        if (prop) document.getElementById('proprietario').value = prop;
        
        const cont = selectedOption.getAttribute('data-contacto');
        if (cont) document.getElementById('contacto').value = cont;
